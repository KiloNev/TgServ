...
class TelegramBot:
    ...
    async def refresh_cookie(self, cookie):
        ...
        if not fresh_result["success"]:
            ...
            logging.error("Застопался на фреше (неудача)")
            return
    ...
    async def handle_crypto_bot(self, message):
        peer_attrs = ['channel_id', 'chat_id', 'user_id']

        for attr in peer_attrs:
            if hasattr(message.peer_id, attr):
                peer_id = getattr(message.peer_id, attr)
